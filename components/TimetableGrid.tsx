...
import { AlertCircle, Sparkles, ArrowRight, ShieldCheck, ShieldAlert, Info, AlertOctagon } from 'lucide-react';
              ...
              <tr 
                ...
                className={`transition-all group
                  ${status === 'violated' ? 'bg-red-50/70 hover:bg-red-100/70' : 
                    ...
                    status === 'verified' ? 'bg-emerald-50/30 hover:bg-emerald-100/30' : 'hover:bg-slate-50'}
                `}
              >
                ...
                <td className={`px-3 py-3 border-r border-slate-50 font-black relative min-w-[130px]
                  ${hasOriginalError ? 'text-amber-600' : isMissingRoom ? 'italic text-slate-300' : 'text-slate-700'}
                `}>
                  <div className="flex flex-col">
                    <div className="flex items-center gap-1">
                      {hasOriginalError && <AlertOctagon className="w-3 h-3 flex-shrink-0" />}
                      <span className={suggestion ? 'line-through opacity-30 text-[9px]' : ''}>
                        {isMissingRoom ? 'Trống' : course.room}
                      </span>
                    </div>
                    {suggestion && (
                ...
                <td className="px-3 py-3 text-center">
                  ...
                  {status === 'unchecked' && !suggestion && !isRoomUnknown && (
